function varargout = panel_revopoint(varargin)
% PANEL_REVOPOINT: Digitize EEG sensors and head shape.
% 
% USAGE:             panel_revopoint('Start')
%                    panel_revopoint('CreateSerialConnection')
%                    panel_revopoint('ResetDataCollection')
%      bstPanelNew = panel_revopoint('CreatePanel')
%                    panel_revopoint('SetSimulate', isSimulate)
...
function Start() %#ok<DEFNU>
    ...
    if (bst_get('iProtocol') <= 0)
        bst_error('Please create a protocol first.', 'Revopoint', 0);
        return;
    end
    % Get subject
    PatientId = java_dialog('input', 'Please, enter subject name or id:', 'Revopoint', [], '');
    if isempty(PatientId)
        return;
    end
    SubjectName = ['Revopoint_', PatientId];
    ...
    db_set_channel(iStudy, ChannelMat, 0, 0);
    % Save condition name
    ...
    view_surface_matrix(sSurf.Vertices, sSurf.Faces, [], sSurf.Color, [], [], OutputSurfacesFiles{end});

    % ===== DISPLAY DIGITIZE WINDOW =====
    % Display panel
    panelContainer = gui_show('panel_revopoint', 'JavaWindow', 'Revopoint', [], [], [], []);
...
function ResetDataCollection(isResetSerial)
    ...
    % Get controls
    ctrl = bst_get('PanelControls', 'Revopoint');
...
function BytesAvailable_Callback(h, ev) %#ok<INUSD>
    ...
    if DigitizeOptions.isSimulate
        ...
        switch (Digitize.Mode)
            ...
            case 4,     pointCoord = [.08 0 0]; % CoordinatesSelector.MRI ./ 1000;
